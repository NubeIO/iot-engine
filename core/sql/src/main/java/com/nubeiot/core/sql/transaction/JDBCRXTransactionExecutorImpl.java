...
final class JDBCRXTransactionExecutorImpl implements JDBCRXTransactionExecutor {
    ...
    @Override
    public <T> Single<T> transactionResult(TransactionalCallable<Single<T>> transactional) {
        return transactionResult0(transactional, dsl().configuration());
    }

    <T> Single<T> transactionResult0(TransactionalCallable<Single<T>> transactional, Configuration configuration) {
        ...
        return Single.just(context)
                     ...
                     .doOnError(cause -> {
                         throw handleRollback(context, provider, listeners, cause);
                     });
    }
