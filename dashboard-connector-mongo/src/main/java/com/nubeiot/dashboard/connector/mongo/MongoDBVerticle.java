package com.nubeiot.dashboard.connector.mongo;

import com.nubeiot.dashboard.connector.mongo.controller.MongoDBController;
import com.nubeiot.core.common.utils.Runner;
import com.nubeiot.core.common.utils.response.ResponseUtils;
import com.nubeiot.core.common.MicroServiceVerticle;
    ...
    private void handleRESTfulRequest(Handler<AsyncResult<HttpServer>> next) {
        ...
        vertx.createHttpServer()
            ...
            .listen(
                    ...
                    config().getInteger("http.port", Port.MONGO_DB_PORT), next);

        publishHttpEndpoint("mongodb-api",
            config().getString("http.host", "0.0.0.0"),
            config().getInteger("http.port", Port.MONGO_DB_PORT),
            ar -> {
                if (ar.succeeded()) {
