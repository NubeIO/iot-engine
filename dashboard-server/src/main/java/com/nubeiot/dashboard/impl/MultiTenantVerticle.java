...
public class MultiTenantVerticle extends RxRestAPIVerticle {
                            ...
                            .flatMap(siteEditedBody -> {
                                if (SQLUtils.in(body.getString("role", ""), Role.USER.toString(), Role.GUEST.toString())) {
                                    ...
                                    if (groupId == null) {
                                        return UserUtils.deleteUser(keycloakUser.getString("id"), accessToken, authServerUrl, realmName, client)
                                            .map(ign -> {
                                                throw HttpHelper.badRequest("You must include group_id on the request data.");
                                            });
                                    }
                                        ...
                                        .flatMap(childUserGroups -> {
                                            if (childUserGroups.size() > 0) {
                                                if (StringUtils.getIdsList(childUserGroups).contains(groupId)) {
                                                    ...
                                                    return UserUtils.deleteUser(keycloakUser.getString("id"), accessToken, authServerUrl, realmName, client)
                                                        .map(ign -> {
                                                            throw HttpHelper.badRequest("<UserGroup> doesn't exist on that <Site>.");
                                                        });
                                                }
                                            } else {
                                                return UserUtils.deleteUser(keycloakUser.getString("id"), accessToken, authServerUrl, realmName, client) // For ADMIN company
                                                    .map(ign -> {
                                                        throw HttpHelper.badRequest("<Site> doesn't have any <UserGroup>.");
                                                    });
                                            }
                                        });
                                } else {
                            ...
                            return mongoClient.rxFindOne(COMPANY, idQuery(associatedCompany.getString("_id")), null)
                                .flatMap(managerLevelCompany -> mongoClient.rxFindOne(COMPANY, idQuery(managerLevelCompany.getString("associated_company_id")), null)
                                    .flatMap(adminLevelCompany -> mongoClient.rxFind(USER, new JsonObject().put("$or", new JsonArray()
                                            ...
                                            .flatMap(users -> dispatchRequests(HttpMethod.PUT, headers, Services.POLICY_PREFIX + siteId, DittoUtils
                                                                                                                                        .createPolicy(users)))
                                            ...
                                            .map(response -> {
                                                if (response != null) {
                                                    if (response.getString("role").equals(Role.MANAGER.toString())
                                                        ...
                                                        throw HttpHelper.badRequest("We should assign Manager level company");
                                                    }
                                                } else {
                                                    throw HttpHelper.badRequest("We don't have the associated_company_id");
                                                }
                                            });
    ...
    private SingleSource<? extends List<JsonObject>> checkPermissionAndReturnValue(Role role, String companyId, List<JsonObject> objects) {
        return Observable.fromIterable(objects)
            .flatMapSingle(jsonObject -> objectLevelPermission(role, jsonObject.getString("associated_company_id"),
                                                               companyId)
                .map(permitted -> {
                    if (!permitted) {
                        throw HttpHelper.forbidden();
                    ...
                    return mongoClient.rxFindOne(COMPANY, idQuery(associatedCompanyId), null)
                        .map(associatedCompany -> {
                            if (associatedCompany != null) {
                                ...
                                throw HttpHelper.badRequest("Requested company doesn't exist on Database.");
                            }
                        })
                        .flatMap(associatedCompany -> {
                            if (associatedCompany.getString("role").equals(Role.MANAGER.toString())) {
                                        ...
                                        return mongoClient.rxFindOne(COMPANY, idQuery(body.getString("associated_company_id", "")), null)
                                            .map(associatedCompany -> {
                                                if (associatedCompany != null
                                                    ...
                                                    throw HttpHelper.badRequest("You can't associated that <Company>!");
                                                }
                                            });
                                    }
                                } else {
