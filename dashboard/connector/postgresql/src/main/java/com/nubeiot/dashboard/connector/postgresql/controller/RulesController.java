...
public class RulesController {
    ...
    public RulesController(Vertx vertx) {
        postgreSQLService = com.nubeiot.dashboard.connector.postgresql.PostgreSQLService.createProxy(vertx,
                                                                                                     com.nubeiot.dashboard.connector.postgresql.PostgreSQLService.SERVICE_ADDRESS);
    }

    public Single<JsonObject> getPostgreSQLData(String query, JsonObject settings) {
        return Single.just(query).flatMap(ignored -> postgreSQLService.rxExecuteQuery(query, settings).map(message -> {
