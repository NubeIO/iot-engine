...
CREATE TABLE IF NOT EXISTS NETWORK (
	...
	CONSTRAINT IDX_UQ_NETWORK UNIQUE ( CODE, EDGE )
 );

CREATE INDEX IDX_NETWORK_EDGE ON PUBLIC.NETWORK ( EDGE );
...
CREATE TABLE IF NOT EXISTS EDGE_EQUIP (
	...
	TIME_AUDIT           varchar(500)   ,
	SYNC_AUDIT           clob(2147483647)   ,
	CONSTRAINT IDX_UQ_EDGE_EQUIP UNIQUE ( EDGE, EQUIP ) ,
	CONSTRAINT PK_EDGE_EQUIP PRIMARY KEY ( ID )
 );

CREATE INDEX IDX_FK_EDGE_EQUIPMENT_EDGE ON EDGE_EQUIP ( EDGE );

CREATE INDEX IDX_FK_EDGE_EQUIP_EQUIP ON EDGE_EQUIP ( EQUIP );

CREATE INDEX IDX_FK_EDGE_EQUIP_NETWORK ON EDGE_EQUIP ( NETWORK );
...
-- ALTER FK
ALTER TABLE EDGE_EQUIP ADD CONSTRAINT FK_EDGE_EQUIPMENT_EDGE FOREIGN KEY ( EDGE ) REFERENCES EDGE( ID ) ON DELETE RESTRICT ON UPDATE CASCADE;

ALTER TABLE EDGE_EQUIP ADD CONSTRAINT FK_EDGE_EQUIP_EQUIP FOREIGN KEY ( EQUIP ) REFERENCES EQUIPMENT( ID ) ON DELETE CASCADE ON UPDATE CASCADE;

ALTER TABLE EDGE_EQUIP ADD CONSTRAINT FK_EDGE_EQUIP_NETWORK FOREIGN KEY ( NETWORK ) REFERENCES NETWORK( ID ) ON DELETE CASCADE ON UPDATE CASCADE;
