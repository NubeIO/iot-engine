...
COMMENT ON TABLE TRANSDUCER IS 'Represents for a set of actual transducers that plugged in edge point. Mean that one or many transducers from one or many equipment, no duplicate and must be same type and category.\nIt can be differenced by type: `SENSOR | ACTUATOR` and categorized by SENSOR kinds or ACTUATOR kinds';
...
CREATE TABLE IF NOT EXISTS PUBLIC.SYNC_DISPATCHER (
	ID                   integer GENERATED ALWAYS AS IDENTITY  NOT NULL,
