...
public class BACnetConfigTest {
    ...
    public void test_deserialize() {
        BACnetConfig config = JsonData.from(
            "{\"vendorId\":1173,\"vendorName\":\"Nube iO Operations Pty Ltd\",\"deviceId\":85084," +
            "\"modelName\":\"NubeIO-Edge28\",\"deviceName\":\"NubeIO-Edge28-85084\",\"maxDiscoverTimeout\":10," +
        ...
        Assert.assertEquals("Nube iO Operations Pty Ltd", config.getVendorName());
        Assert.assertEquals("NubeIO-Edge28", config.getModelName());
        Assert.assertEquals(85084, config.getDeviceId());
        Assert.assertEquals("NubeIO-Edge28-85084", config.getDeviceName());
    }
